...
function snakeForeheadGeometry() {
  ...
  const foreheadBufferAttribute = new THREE.BufferAttribute(
    foreheadVertices,
    3
  );
  const foreheadGeometry = new THREE.BufferGeometry();
  foreheadGeometry.setAttribute("position", foreheadBufferAttribute);
  const forehead = new THREE.Mesh(foreheadGeometry, material1);
  forehead.position.set(1.5, 0, 1.5);
