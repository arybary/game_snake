...
function snakeHeadBackGeometry() {
  ...
  const headBackBufferAttribute = new THREE.BufferAttribute(
    headBackVertices,
    3
  );
  const headBackGeometry = new THREE.BufferGeometry();
  headBackGeometry.setAttribute("position", headBackBufferAttribute);
  const headBack = new THREE.Mesh(headBackGeometry, material1);
  headBack.position.set(1.5, 0, 1.5);
