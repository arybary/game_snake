...
function snakeNoseGeometry() {
  ...
  const noseBufferAttribute = new THREE.BufferAttribute(noseVertices, 3);
  const noseGeometry = new THREE.BufferGeometry();
  noseGeometry.setAttribute("position", noseBufferAttribute);
  const nose = new THREE.Mesh(noseGeometry, material1);
  nose.position.set(1.5, 0, 1.5);
