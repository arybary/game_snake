...
function snakeNoseTipGeometry() {
  ...
  const noseTipBufferAttribute = new THREE.BufferAttribute(noseTipVertices, 3);
  const noseTipGeometry = new THREE.BufferGeometry();
  noseTipGeometry.setAttribute("position", noseTipBufferAttribute);
  const noseTip = new THREE.Mesh(noseTipGeometry, material1);
  noseTip.position.set(1.5, 0, 1.5);
